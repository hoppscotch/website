    ...
    <SmartLink :to="`blog/${blog.slug}`" class="flex-col w-full h-full">
      ...
      <div class="flex flex-col flex-1 p-8">
        ...
        <div class="flex items-center my-4">
          <img class="w-8 h-8 rounded-full" :src="blog.author.image" :alt="blog.author.name">
        ...
        <div class="relative w-full my-2 overflow-hidden faded-edge">
          <div class="flex w-full space-x-2 overflow-x-auto flex-nowrap">
            <span v-for="(tag, tagIndex) in blog.tags" :key="`tag-${tag}-${tagIndex}`" class="px-3 py-2 my-1 text-xs rounded-full bg-primaryDark border-primaryDark border-1 whitespace-nowrap">
              {{ t(tag) }}
            </span>
          </div>
        </div>
...
.faded-edge::after{
  position:absolute;
  background-image: linear-gradient(to right, var(--color-primaryLight) 0%, rgba(255,255,255,0) 100%);
  content: '';
  height: 100%;
  width: 32px;
  z-index: 2;
