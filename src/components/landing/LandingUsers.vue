<script setup lang="ts">
import { users } from "~/assets/data/landingUsers"
import { isDark } from "~/composables"

const { t } = useI18n()
</script>

<template>
  <div class="flex flex-col px-8 py-16">
    <div
      class="my-4 font-semibold tracking-widest text-center uppercase text-accentLight"
    >
      {{ t("home.users.title") }}
    </div>
    <div
      class="mx-auto mt-8 grid grid-cols-3 gap-8 md:grid-cols-4 lg:max-w-4xl lg:grid-cols-6"
    >
      <div
        v-for="(user, index) in users"
        :key="`user-${index}`"
        class="inline-flex flex-col items-center justify-center p-4 transition hover:opacity-100"
      >
        <img
          :src="`/assets/images/users/${isDark ? 'dark' : 'light'}-${
            user.image
          }`"
          :alt="user.title"
          :title="user.title"
          loading="lazy"
          class="inline-flex flex-col object-cover object-center max-h-16"
        />
      </div>
    </div>
  </div>
</template>
